...
@section('content')
    ...
    {!! Form::model($mercado, ['route' => ['mercadoadmin.update', $mercado->id], 'method' => 'PUT', 'files'=> true ]) !!}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            {{ Form::label('users', 'Productos',['class'=>'form-spacing-top']) }}
                            {{ Form::select('users[]',$users, null, ['class'=>'form-control select2-multis', 'multiple'=>'multiple']) }}
                        </div>
                    </div>
...
@section('after_scripts')
  ...
  <script type="text/javascript">
    ...
    $('.select2-multis').select2().val({!! json_encode($mercado->users()->getRelatedIds()) !!}).trigger('change');
  </script>
